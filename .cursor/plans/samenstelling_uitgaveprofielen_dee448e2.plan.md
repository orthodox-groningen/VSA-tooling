...
todos:
  - id: todo-1-3-done
    ...
    status: pending
  - id: spec-secties
    content: Voeg concepten, uitgaveprofielen, kanalen, bron-repo-mapping en nesting-besluit toe aan spec-vsa-document-samenstellen.md
    status: pending
  - id: include-channels
...
# Samenstelling, uitgaveprofielen en kanaal-includes
...
- [docs/spec-vsa-document-samenstellen.md](docs/spec-vsa-document-samenstellen.md) beschrijft al **één HTML-pipeline** met print-CSS (uitgaveprofiel Afdruk ≈ Online + `@media print`).
...
- [docs/todo.md](docs/todo.md) §2.2 (gegeneraliseerde include) en §2.3 (nesting) zijn de logische vervolgstappen onder deze paraplu.
...
**Housekeeping (gevraagd):** markeer [docs/todo.md](docs/todo.md) §1.3 als `Afgerond` — halftoon-prefixen zijn geïmplementeerd en getest ([tests/test_halftoon_prefix.py](tests/test_halftoon_prefix.py), validatie in [src/vsa/validation_runner.py](src/vsa/validation_runner.py)).
...
**Aanpak:** uitbreiden van [docs/spec-vsa-document-samenstellen.md](docs/spec-vsa-document-samenstellen.md) (niet hernoemen in increment 1 — titel kan later naar *VSA Samenstelling en Uitgave* als de scope breder wordt).
...
| Authoring-conventie voor titels/navigatie                                   | `web-only` als **sibling vóór** `keep-together` (zoals [zondag-toon-3.md](examples/hugo-demo/content-source/praktijk/zondagen/zondag-toon-3.md)) |
...
Toevoegen aan [docs/spec-vsa-document-samenstellen.md](docs/spec-vsa-document-samenstellen.md):
...
- [docs/todo.md](docs/todo.md): §1.3 → `Afgerond`; §2.2 → gedeeltelijk (`svg`/`coria`/`mxl` alias; volledige bron-repo-default nog open)
