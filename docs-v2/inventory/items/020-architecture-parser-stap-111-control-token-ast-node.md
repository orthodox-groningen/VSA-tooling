# Inventarisatie 020 — Stap 111 - ControlTokenNode in AST

## Huidige locatie

`docs/architecture/parser-stap-111-control-token-ast-node.md`

## Documenttype

Historie / ontwikkelstap

## Doelgroep

Nog te bevestigen.

## Inhoudelijke samenvatting

Control tokens krijgen een eigen AST-node voordat parserdispatch wordt gebouwd. meaning="phrase_boundary", Deze node legt nog geen renderergedrag vast.

## Belangrijke koppen

- `# Stap 111 - ControlTokenNode in AST`
- `## Doel`
- `## Node`
- `## Abstracte betekenissen`
- `## Belangrijk`
- `## Nog niet gedaan`

## Relaties met andere documenten

Nog te bepalen tijdens de overlap-analyse.

## Overlap met andere documenten

Nog te bepalen tijdens de overlap-analyse.

## Voorgestelde bestemming

`docs-v2/history/parser/parser-stap-111-control-token-ast-node.md`

## Status inventarisatie

Eerste inventarisatie op basis van bestandsnaam, koppen en inhoudsscan.

## Opmerkingen

Controleer in fase 1 of dit document normatieve inhoud bevat die niet verloren mag gaan bij latere samenvoeging.
