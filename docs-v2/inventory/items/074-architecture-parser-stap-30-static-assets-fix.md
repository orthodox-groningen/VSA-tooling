# Inventarisatie 074 — Stap 30 - static assets fix

## Huidige locatie

`docs/architecture/parser-stap-30-static-assets-fix.md`

## Documenttype

Historie / ontwikkelstap

## Doelgroep

Nog te bevestigen.

## Inhoudelijke samenvatting

De demo-site toonde wel tekst en invoer, maar geen SVG-plaatjes. <img src="/vsa/voorbeeld.svg"> verwijst naar een URL onder Hugo static output.

## Belangrijke koppen

- `# Stap 30 - static assets fix`

## Relaties met andere documenten

Nog te bepalen tijdens de overlap-analyse.

## Overlap met andere documenten

Nog te bepalen tijdens de overlap-analyse.

## Voorgestelde bestemming

`docs-v2/history/parser/parser-stap-30-static-assets-fix.md`

## Status inventarisatie

Eerste inventarisatie op basis van bestandsnaam, koppen en inhoudsscan.

## Opmerkingen

Controleer in fase 1 of dit document normatieve inhoud bevat die niet verloren mag gaan bij latere samenvoeging.
