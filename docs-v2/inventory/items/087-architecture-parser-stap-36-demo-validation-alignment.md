# Inventarisatie 087 — Stap 36 - demo/validator alignment

## Huidige locatie

`docs/architecture/parser-stap-36-demo-validation-alignment.md`

## Documenttype

Historie / ontwikkelstap

## Doelgroep

Nog te bevestigen.

## Inhoudelijke samenvatting

Stap 36 introduceerde een nieuwe semantische regel: is ongeldig na gezongen materiaal. - Hugo demo voorbeelden;

## Belangrijke koppen

- `# Stap 36 - demo/validator alignment`

## Relaties met andere documenten

Nog te bepalen tijdens de overlap-analyse.

## Overlap met andere documenten

Nog te bepalen tijdens de overlap-analyse.

## Voorgestelde bestemming

`docs-v2/history/parser/parser-stap-36-demo-validation-alignment.md`

## Status inventarisatie

Eerste inventarisatie op basis van bestandsnaam, koppen en inhoudsscan.

## Opmerkingen

Controleer in fase 1 of dit document normatieve inhoud bevat die niet verloren mag gaan bij latere samenvoeging.
