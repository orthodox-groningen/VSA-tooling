...
glossaryNotes:
  ...
  - "[Pitch-markers](@) / [hoogte-markeringen](@) en [control-tokens](@) kunnen eveneens als glyphs of layout-aanwijzingen worden weergegeven."
---
...
Goede/valide voorbeelden van Glyph zijn:
- Strepen, punten, connectors van [modifiers](@)
- Visuele weergave van een [pitch-marker](@) / [hoogte-markering](@)

Geen goede/niet valide voorbeelden van Glyph zijn:
- De letters in `Hei` binnen `{/Hei_}`
- De brontekens `/` of `_` als platte ASCII in de `.vsa`

## Motivatie

[Parser](@) en [validator](@) werken op tokens; lezers zien strepen en punten.
Het glyphbegrip scheidt die visuele laag van de syntactische tokens, zodat
