...
def test_nested_markdown_asset_stem():
...
def test_public_html_asset_stem():
    mod = load_repair_module()
...
def test_rewrite_wrong_img_ref_to_expected_nested_ref():
    mod = load_repair_module()

    html = '<img class="vsa-notation" src="/vsa/voorbeelden-praktijk-donderdag-block-1.svg" alt="VSA notatie">'
    fixed = mod.rewrite_img_refs(html, "voorbeelden-praktijk-weekdagen-woensdag")

    assert '/vsa/voorbeelden-praktijk-weekdagen-woensdag-block-1.svg' in fixed
